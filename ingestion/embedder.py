...
def load_embedding_model(model_name: str = EMBEDDING_MODEL) -> SentenceTransformer:
    ...
    print(f"  ✅ Model loaded | Embedding dimensions: {dim}")
    return model
...
def get_chroma_client(db_path: str = CHROMA_DB_PATH) -> chromadb.PersistentClient:
    ...
    print(f"\n🗄️  ChromaDB connected at: {db_path}")
    return client
...
def embed_and_store_chunks(
    ...
    if not new_chunks:
        print("  ✅ All chunks already embedded — nothing new to add")
        return
    ...
    print(f"\n✅ ChromaDB now contains {collection.count()} embedded chunks")

def verify_embedding(
    collection : chromadb.Collection,
    model      : SentenceTransformer,
    test_query : str = "motor overheating fault"
) -> None:
    ...
    for i, (doc, meta, dist) in enumerate(zip(
        ...
        print(f"       Manual     : {meta['manual_name']}")
...
if __name__ == "__main__":
    ...
    if not chunks_file.exists():
        ...
        verify_embedding(collection, model)